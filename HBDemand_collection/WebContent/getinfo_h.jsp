...
<body>
	...
	<%
		...
		try {
			...
			if (flag.equals("1")) {
				rs = db.executeQuery(
						"select secondID,secondName from second_h where father='" + value + "' order by secondID");
				while (rs.next()) {
				...
				rs = db.executeQuery(
						"select thirdID,thirdName from third_h where father='" + ff + "' order by thirdID");
				...
				rs = db.executeQuery(
						"select thirdID,thirdName from third_h where father='" + value + "' order by thirdID");
				while (rs.next()) {
					values.append(rs.getString(1) + "," + rs.getString(2) + ";");
				}
			} else {
